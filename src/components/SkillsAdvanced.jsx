...
const SkillSegment = ({ level, color, isDarkMode, inView, skillIndex }) => {
    return (
        <div className="relative">
            <div className={`h-3 rounded-full overflow-hidden flex gap-1 p-[2px] ${isDarkMode ? 'bg-white/5' : 'bg-slate-200/50'}`}>
                {[...Array(10)].map((_, i) => (
                    <div key={i} className={`flex-1 rounded-full ${isDarkMode ? 'bg-white/5' : 'bg-white/40'}`} />
                ))}
            </div>
            ...
            <motion.div
                ...
                transition={{ duration: 1, ease: "circOut", delay: 0.2 + (skillIndex * 0.05) }}
            >
                {[...Array(10)].map((_, i) => (
                    <div
                        key={i}
                        ...
                        style={{
                            ...
                            boxShadow: i < (level / 10) ? `0 0 12px ${color}60` : 'none'
                        }}
                    >
                        {i < (level / 10) && (
        ...
        <section id="skills" className="container-custom relative overflow-hidden">
            ...
            >
                <div className="text-center space-y-4">
                    <motion.div variants={itemVariants} className={`inline-block px-4 py-1 rounded-full text-[10px] font-black tracking-widest uppercase border ${isDarkMode ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400' : 'bg-indigo-50 border-indigo-100 text-indigo-600'}`}>
                        Technology Stack
                    </motion.div>
                    ...
                    <motion.p variants={itemVariants} className={`text-lg max-w-2xl mx-auto font-medium ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                    ...
                    {skillCategories.map((category, catIndex) => (
                        ...
                        >
                            <div className="flex items-center gap-6">
                                <div className={`w-16 h-16 rounded-3xl flex items-center justify-center text-3xl shadow-2xl transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6 ${isDarkMode ? 'bg-slate-800/80 border border-white/10' : 'bg-indigo-50 border border-indigo-100'}`}>
                                    {category.icon}
                                </div>
                                    ...
                                    <div key={skillIndex} className="space-y-4">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                <div className="p-2 rounded-xl bg-white/5 border border-white/5 transition-colors group-hover:border-indigo-500/20">
                                                    <skill.icon className="text-2xl" style={{ color: skill.color }} />
                                                </div>
                                                <span className={`font-black uppercase text-xs tracking-[0.2em] ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>{skill.name}</span>
                                            </div>
                ...
                >
                    {[
                        ...
                        <div key={idx} className="glass-card px-8 py-5 rounded-3xl flex flex-col items-center gap-1 group hover:border-indigo-500/50">
                            ...
                            <span className={`text-base font-black ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{badge.value}</span>
                        </div>
                    ))}
                </motion.div>
            </motion.div>
